...
# 建立一個 Flask 應用實例，__name__ 代表目前這個 Python 檔案的名稱。
...
# 指定游標類型為 DictCursor，代表查詢資料會以字典格式回傳（key 是欄位名稱）
...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        cur.execute('SELECT * FROM users WHERE username=%s AND password = %s',
                    (username, password))  # 去 users 資料表找出符合帳號和密碼的那一筆資料。
        ...
        if isuser:
            ...
            flash('帳號或密碼錯誤，請重新輸入')
            # 將使用者導回登入頁面（login_page() 函式所對應的 / 頁面）
...
def index_page():
    ...
    if user_id:
        # 只查詢這位醫生的這位病患
        cur.execute(
            "SELECT * FROM patients WHERE doctor_id = %s AND patient_id = %s", (doctor_id, user_id))
    else:
        # 查詢這位醫生的所有病患
    ...
    # 先建一個字典，預設所有風險等級數量都設為 0，避免資料庫查不到某個等級時出錯。
    risk_counts = {'高風險': 0, '中風險': 0, '低風險': 0}
